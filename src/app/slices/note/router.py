...
@router.get('/{id}', response_model=NotePublic, responses=generate_openapi_error_responses({404}))
async def read_note(id: uuid.UUID, session: SessionDep):
    note = await get_or_404(session, id)
    return NotePublic.model_validate(note)


@router.post('/', response_model=NotePublic)
async def create_note(*, session: SessionDep, note_in: NoteCreate):
    tags = await get_or_create_tags(session, note_in.owner_id, note_in.tags)
    note = Note(
        name=note_in.name,
        content=note_in.content,
        tags=tags,
        owner_id=note_in.owner_id,
    )
    session.add(note)
...
async def update_note(*, session: SessionDep, id: uuid.UUID, note_in: NoteUpdate):
    note = await get_or_404(session, id)
...
async def delete_note(*, session: SessionDep, id: uuid.UUID):
    note = await get_or_404(session, id)
